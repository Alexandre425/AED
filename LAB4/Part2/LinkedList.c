 ...
 */
LinkedList * insertSortedLinkedList(LinkedList * first, Item item, int (*cmp) (Item item1, Item item2), int * err)
{
  LinkedList *new;
  LinkedList *aux = first, *last = NULL;
  ...
  while(aux != NULL && cmp(item, aux->this) == 0){
    last = aux;
    aux = aux->next;
  }
  /* head insertion */
