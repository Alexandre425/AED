...
struct _heap
{
  ...
  void (*free_f)(Item);    /* function pointer to item freeing function */
...
Heap *NewHeap(int size, int (*less)(Item, Item), void (*print)(Item), void (*free_f)(Item))
{
  Heap *h;
...
void CleanHeap(Heap *h)
{
  for (int i = 0; i < h->n_elements; i++)
    if (h->heapdata[i] != NULL)
    {
      (h->free_f)(h->heapdata[i]);
      h->heapdata[i] = NULL;
    }
  h->n_elements = 0;
...
int VerifyHeap(Heap *h)
{
  ...
  for (int i = 1; i < h->n_elements; i++)
    if ((h->less)(h->heapdata[parent(i)], h->heapdata[i]))
    {
      status = 0;
      break;
    }
...
void Heapify(Heap *h)
{
  for (int i = parent(h->n_elements); i >= 0; i--)
    FixDown(h, i);

  return;
}

void exch(Heap *h, int a, int b){
  Item tmp = h->heapdata[a];
  h->heapdata[a] = h->heapdata[b];
  h->heapdata[b] = tmp;
}